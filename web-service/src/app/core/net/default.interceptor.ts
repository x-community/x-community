...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  private checkStatus(ev: HttpResponseBase) {
    ...
    this.notification.error(`Woops`, errortext);
  }
      ...
      default:
        if (ev instanceof HttpErrorResponse) {
          this.blobToText(ev.error).subscribe(resp => {
            ...
            this.notification.error('Error', err && err.message ? err.message : 'Internal Server Error', { nzDuration: 5000 });
          });
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    return next.handle(req).pipe(mergeMap((ev) => {
      // 允许统一对请求错误处理
